...
#Authorization IjA1NjE4ODhhZjQyMzExZWY5N2FjMDI0MmFjMTIwMDA0Ig.Z77dCA.PfvYsCoF2Erriar5Ns5u0KOqUck
#session=mZxQrjaGNer3ljobWM6_a06YKbJcL1ayQV4N6parDhw
@manager.route('/completionnew', methods=['POST'])  # noqa: F821
@validate_request("conversation_id", "messages")
def completionnew():
    print("In conversation starts completionnew")
    #token = request.headers.get('Authorization').split()[1]
    objs = APIToken.query(token="IjA1NjE4ODhhZjQyMzExZWY5N2FjMDI0MmFjMTIwMDA0Ig.Z77dCA.PfvYsCoF2Erriar5Ns5u0KOqUck")
    #if not objs:
    #    return get_json_result(
     #       data=False, message='Authentication error: API key is invalid!"', code=settings.RetCode.AUTHENTICATION_ERROR)
    req = request.json
