...
@manager.route('/listall', methods=['GET'])  # noqa: F821
@login_required
def list_all_files():
    pf_id = request.args.get("parent_id")
    ...
    desc = request.args.get("desc", True)
    if not pf_id:
        root_folder = FileService.get_all_folders()

        print(f"Total folders :{str(len(root_folder))}")
        first_record = root_folder[0] if root_folder else None  # Check if list has elements

        root_folder = first_record
    ...
    try:
        ...
        all_files, file_count = FileService.get_all_file_list(page_number, items_per_page,
                                                              orderby, desc, keywords)
        print(f"All files {str(len(all_files))} ,, {str(file_count)}")
        return get_json_result(data={"total": file_count, "files": all_files, "parent_folder": ""})
...
def get(file_id):
    try:
        ...
        print(f"Error in file get {str(e)}")
        return server_error_response(e)
