...
def list_kbs():
    ...
    try:
        ...
        kbs, total = KnowledgebaseService.get_by_tenant_ids(
            [m["tenant_id"] for m in tenants], None, page_number,
       ...
       # print("All tenants", all_kbs)
        #print("All tenants", total_count)
        return get_json_result(data={"kbs": all_kbs, "total": total_count})

        '''kbs, total = KnowledgebaseService.get_by_tenant_ids(
            [m["tenant_id"] for m in tenants], current_user.id, page_number,
            items_per_page, orderby, desc, keywords, parser_id)
        print("All tenants", total)
        #return get_json_result(data={"kbs": [], "total": 0})
