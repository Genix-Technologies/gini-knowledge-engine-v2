...
class FileService(CommonService):
    ...
    def get_all_file_list(cls, page_number, items_per_page,
                     orderby, desc, keywords):
        if keywords:
            files = cls.model.select().where((fn.LOWER(cls.model.name).contains(keywords.lower())),
                                             cls.model.type != FileType.FOLDER.value)
        else:
            files = cls.model.select().where(cls.model.type != FileType.FOLDER.value)
        count = files.count()
        if desc:
