...
class TenantLLMService(CommonService):
    ...
    def model_instance(cls, tenant_id, llm_type,
                       llm_name=None, lang="English"):
        model_config = TenantLLMService.get_model_config(tenant_id, llm_type, llm_name)
        if llm_type == LLMType.EMBEDDING.value:
            print("In llm service.py 134 line ::", model_config["llm_factory"])
            print(model_config)
...
class LLMBundle(object):
    ...
    def __init__(self, tenant_id, llm_type, llm_name=None, lang="English"):
        ...
        print("In LLM Service = 235", self.mdl)
        assert self.mdl, "Can't find model for {}/{}/{}".format(
            tenant_id, llm_type, llm_name)
        model_config = TenantLLMService.get_model_config(tenant_id, llm_type, llm_name)
        print("Foudn model config", model_config)
