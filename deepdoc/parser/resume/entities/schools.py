...
TBL = pd.read_csv(
    os.path.join(current_file_path, "res/schools.csv"), sep="\t", header=0, encoding="utf-8"
).fillna("")
if "name_en" in TBL.columns:
    TBL["name_en"] = TBL["name_en"].map(lambda x: x.lower().strip() if isinstance(x, str) else "")
