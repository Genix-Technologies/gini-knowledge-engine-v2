...
class ESConnection(DocStoreConnection):
    def __init__(self):
        self.info = {}
        logger.info(f"Use Elasticsearch  as the doc engine.") #{settings.ES['hosts']}
        for _ in range(ATTEMPT_TIME):
            try:
                ...
                logger.warning(f"{str(e)}. Waiting Elasticsearch  to be healthy.") #{settings.ES['hosts']}
                time.sleep(5)
        if not self.es.ping():
            msg = f"Elasticsearch  is unhealthy in 120s." #{settings.ES['hosts']}
            logger.error(msg)
            raise Exception(msg)
        ...
        logger.info(f"Elasticsearch  is healthy.") #{settings.ES['hosts']}
